  ...
  def calcVector(self, vector, avoidance, controlFine, controlFineDistance):
    ...
    if controlFine: 
      ...
      if self.countingDistanceOutsideBool:
        ...
        time_difference = (datetime.datetime.now() - self.start_time_counter).total_seconds() 

        if time_difference < 10:
          # print(controlFineDistance)
          self.stoneError.append(controlFineDistance)
        else:
          if self.printStoneCenteredBool:
            array_sum = sum(self.stoneError)
            array_average = array_sum / len(self.stoneError)
      ...
      if controlFineDistance < (((3)/1000)/100)*32768: 
        ...
        if self.timeCountingBool2:
          ...
          print((datetime.datetime.now() - self.start_time2).total_seconds())
