...
class kidneyStoneNavigationWidget:
  ...
  def setup(self):
    ...
    self.forwardAdvanceThresholdBox.setValue(5)
    ...
    self.parameterBBox.setValue(40)
    ...
    self.parameterYBox.setValue(40)
    ...
    self.parameterXBox.setDecimals(4)
  ...
  def mainCodeRunner(self):
    ...
    # Calculate the region boundaries for the middle 50% of the image
    middle_50_percent_width = contrastImage.shape[1] * 0.25
    middle_50_percent_height = contrastImage.shape[0] * 0.25
    top_left = (int(img_center_x - middle_50_percent_width), int(img_center_y - middle_50_percent_height))
    bottom_right = (int(img_center_x + middle_50_percent_width), int(img_center_y + middle_50_percent_height))
    ...
    if contrast >= contrast_threshold:
      ...
      _, binary_mask = cv2.threshold(contrast_frame, 150, 255, cv2.THRESH_BINARY)
      ...
      # Draw the center of the contour in green
      cv2.circle(contrastImage, (cX, cY), 5, (0, 255, 0), -1)

      distance_to_center = np.sqrt((cX - img_center_x) ** 2 + (cY - img_center_y) ** 2)
      ...
      if distance_to_center < self.brightSpotDistanceBox.value:
        ...
        cv2.drawContours(contrastImage, [largest_contour], -1, (0, 0, 255), 2)
  ...
  def calcVector(self, vector, avoidance, controlFine, controlFineDistance):
    ...
    if controlFine: 
      ...
      if controlFineDistance < (((self.DeadzoneFineBox.value)/1000)/100)*32768: 
        distance = 0
        print("distance is 0")
